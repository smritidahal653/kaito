...
func (r *ResourceSpec) validateCreateWithInference(inference *InferenceSpec) (errs *apis.FieldError) {
	...
	if skuConfig, exists := gpuConfigs[instanceType]; exists {
		...
		if provider == consts.AWSCloudName || (provider == consts.AzureCloudName && !strings.HasPrefix(instanceType, N_SERIES_PREFIX) && !strings.HasPrefix(instanceType, D_SERIES_PREFIX)) {
